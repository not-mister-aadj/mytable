...
import { sampleSundayTableCancelProps } from "@/emails/sample-data";
import { requireAdmin } from "@/lib/admin-auth";
import { sundayTableCancelSubject } from "@/lib/email/subjects";

export default async function SundayTableCancelEmailPreviewPage() {
  await requireAdmin();
  const html = await render(
    SundayTableCancelEmail(sampleSundayTableCancelProps),
  );
  const subject = sundayTableCancelSubject(
      ...
      <div className="mb-6 flex items-center justify-between">
        <div>
          <h1 className="font-serif text-2xl text-burgundy">
            E-mail preview · Sunday Table annulering
          </h1>
