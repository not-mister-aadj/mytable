...
import { sampleSundayTableLocationProps } from "@/emails/sample-data";
import { requireAdmin } from "@/lib/admin-auth";
import { sundayTableLocationSubject } from "@/lib/email/subjects";

export default async function SundayTableLocationEmailPreviewPage() {
  await requireAdmin();
  const html = await render(
    SundayTableLocationEmail(sampleSundayTableLocationProps),
  );
  const subject = sundayTableLocationSubject(
