...
export function GirlsOnlyHeader({
  ...
  return (
    <header
      className={`girls-only-header fixed inset-x-0 top-0 z-50 backdrop-blur-md transition-all duration-300 ${
        scrolled || menuOpen
    ...
    >
      <div className="mx-auto flex max-w-7xl items-center justify-between gap-4 px-5 py-3.5 sm:px-8 lg:px-10">
        <Link
          href={pageRoot}
          className="girls-only-header__logo relative z-50 shrink-0 transition-opacity hover:opacity-90"
        ...
        <div className="flex items-center gap-2 lg:hidden">
          ...
          <button
            type="button"
            className="girls-only-header__menu-btn relative z-50 flex h-10 w-10 flex-col items-center justify-center gap-1.5 rounded-full border"
      ...
      <nav
        className={`fixed inset-0 z-40 flex flex-col bg-gradient-to-b from-[var(--go-blush-mist)] via-[var(--go-cream)] to-[var(--go-cream)] transition-all duration-300 lg:hidden ${
          menuOpen
